...
function Home() {
  ...
  const movies = movie.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={overview} poster={data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });


  useEffect(() => {
    fetch('https://mymoviz-back-one.vercel.app/movies')
